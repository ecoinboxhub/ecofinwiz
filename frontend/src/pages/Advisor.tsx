    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-lg mx-auto">
      <div className="px-4 py-3 flex items-center justify-between border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="font-semibold text-gray-800 text-sm">{t("advisor.name")}</h1>
            <p className="text-xs text-gray-400">{t("advisor.role")}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4" role="log" aria-live="polite">
        {messages.length === 0 && !loading && !streamingContent && (
          <div className="text-center py-10">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-sky-400 to-sky-600 flex items-center justify-center mx-auto mb-4">
              <Sparkles className="w-8 h-8 text-white" />
            </div>
            <h2 className="font-semibold text-gray-700 mb-2">{t("advisor.welcomeTitle")}</h2>
            <p className="text-sm text-gray-400 max-w-xs mx-auto">{t("advisor.welcomeDesc")}</p>
            <div className="flex flex-wrap gap-2 justify-center mt-4">
              {[t("advisor.question1"), t("advisor.question2"), t("advisor.question3"), t("advisor.question4")].map((q) => (
                <button key={q} onClick={() => setInput(q)} className="text-xs bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-full text-gray-600 hover:bg-sky-50 hover:border-sky-200 transition-colors">
                  {q}
                </button>
              ))}
            </div>
        ...
        {allMessages.map((m) => (
          <div key={m.id} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${m.role === "user" ? "bg-sky-500 text-white rounded-br-lg" : "bg-gray-100 text-gray-700 rounded-bl-lg"}`}>
